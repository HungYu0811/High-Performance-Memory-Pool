...
// v1.9: in_use is now vector<uint8_t> (was vector<bool>) — see file header comment.
template <typename T>
...
private:
    ...
    void grow_pool() {
        ...
        for (size_t i = 0; i < maxSlots; ++i) in_use.push_back(0);
    ...
    void* allocate() {
        ...
        for (size_t i = 0; i < memory_list.size(); ++i) {
            ...
            if ((char*)p >= c && (char*)p < c + myChunkSize) {
                ...
                in_use[i * maxSlots + local_index] = 1; // v1.9: was `true`
                break;
            }
        }
        return p;
...
int main() {
    ...
    std::cout << "=== High-Performance Memory Pool - v1.9 benchmark ===\n";
