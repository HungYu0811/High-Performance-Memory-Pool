...
private:
    ...
    // same order chunks were appended to memory_list. 1 = block is currently on loan to a
    // caller; 0 = block is sitting in the free list (or hasn't been carved out yet).
    //
    // v1.9: vector<bool> -> vector<uint8_t>. vector<bool> is a bit-packed specialization, not a
    // real Container (operator[] returns a proxy object, not a bool&), so every access costs an
    // extra shift/mask. This pool's access pattern is single-index random access on the hot path
    // (allocate()/deallocate() touch exactly one entry per call, never a bulk scan), so it never
    // benefits from the bit-packing density and only pays its per-access cost. Memory cost of the
    // switch is small in absolute terms: 1 byte vs 1 bit per block, against a 64-byte blockSize,
    // i.e. ~1.5% extra footprint per block. Measured via interleaved benchmarking against v1.8 on
    // a 6-thread VM: +17.7% median single-threaded Mops/sec (7/8 rounds), +12.0% median
    // multi-threaded Mops/sec (6/8 rounds) — see README v1.9 benchmark addendum.
    ...
    void grow_pool(){
        ...
        // out free (0) — nothing has been handed out of this chunk yet. Appending (rather than
        // resizing up front) keeps in_use's size in lockstep with how many chunks actually exist.
        for (size_t i = 0; i < maxSlots; ++i) {
            in_use.push_back(0); // v1.9: was `false`
        }
...
public:
    ...
    void deallocate(void* address) {
        ...
        // allocate(): chunk_number * maxSlots + local_index. This lets a single flat vector
        // (vector<uint8_t> as of v1.9, was vector<bool>) represent every block across every chunk
        // this pool owns, without index collisions between chunks (each chunk occupies its own
        // maxSlots-wide band in in_use).
        ...
        in_use[index] = 0; // v1.8: mark free only after the double-free check passes. (v1.9: was `false`)
