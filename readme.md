...
### v1.9 : `in_use` Storage Switched from `vector<bool>` to `vector<uint8_t>`

#### Design / Changes (relative to v1.8):
1. **`std::vector<bool> in_use` → `std::vector<uint8_t> in_use`.** Semantics are unchanged (0/non-zero = free/on-loan); only the backing storage changes. `vector<bool>` is a bit-packed specialization, not a real STL Container — `operator[]` returns a proxy object rather than a `bool&`, so every read or write costs an extra shift/mask against the packed byte, on top of the memory access itself.
2. **Why this pool doesn't benefit from bit-packing:** `in_use` is only ever touched via single-index random access, once per `allocate()`/`deallocate()` call — never scanned in bulk. Bit-packing's advantage (higher density, fewer cache lines touched during a scan) only pays off for sequential/bulk access patterns; a single-index hot path just pays the packing's per-access overhead with none of its benefit.
3. **Memory cost is negligible here:** the switch costs 1 byte per block instead of 1 bit, against a 64-byte `blockSize` — roughly 1.5% extra footprint per block, not a meaningful regression for this pool's scale.
4. **Everywhere `in_use` was set to `true`/`false` is now `1`/`0`** (`grow_pool()`'s initial fill, `allocate()`'s mark-in-use, `deallocate()`'s mark-free); no other logic changed.

#### Verified:
* `main.cpp`'s `SimpleMemoryPool<T>` compiles cleanly (`g++ -std=c++17 -O2 -Wall -Wextra`) with the `vector<uint8_t>` change — no warnings introduced.
* **Still to do:** the double-free regression test (`dfree_test.cpp` — free `p` twice, then allocate twice and confirm `second == p + blockSize`, not `p`) needs to be re-run against this updated `main.cpp` to confirm the storage-type switch didn't alter double-free-detection behavior. An earlier version of this test was reportedly run against a `vector<uint8_t>` pool, but since `main.cpp` itself was still on `vector<bool>` until this edit, it's not certain that earlier run was against *this* file — worth re-confirming rather than assuming. See Next Steps below.

#### Measured (interleaved benchmark, 6-thread VM, 8 rounds — see the v1.9 benchmark addendum below for full methodology):
* Single-threaded: **+17.7%** median Mops/sec vs v1.8 (95.9 → 112.9 Mops/sec), 7/8 rounds favoring v1.9.
* Multi-threaded (6 threads): **+12.0%** median Mops/sec vs v1.8 (460.8 → 516.0 Mops/sec), 6/8 rounds favoring v1.9.

#### Known limitations carried into this version (relative to v1.8):
* **Resolved:** `vector<bool>`'s per-access bit-packing overhead on the hot path — replaced with `vector<uint8_t>`, measured improvement above.
* **Still open:** optimization B (magic-number fast-division for the `off / blockSize` line) remains deferred — `blockSize` is not guaranteed to be a power of two (the current `sizeof(Player)=48 → blockSize=64` case is coincidental to this particular `T`), so a bit-shift substitution isn't valid without a real magic-number-division implementation. See the open item at the end of the v1.9 benchmark addendum below.
* **Everything else unchanged from v1.8** — same locking characterization, same double-free/bounds/alignment checks, same zero-metadata departure (now 1 byte/block instead of 1 bit/block).
...
**Update — see the v1.9 addendum below.** The "not directly comparable" environment caveat two paragraphs up is superseded by a clean same-machine re-run of this same v1.7-vs-v1.8 comparison, done alongside the v1.8-vs-v1.9 chain on a new 6-hardware-thread VM.

---

#### Benchmark addendum: v1.9 vs v1.8, and a clean same-machine v1.7→v1.8→v1.9 chain (6-thread VM)

The user switched to a new 6-hardware-thread VM and re-ran the full interleaved chain — `./interleaved_bench.sh ./benchmark_v1_7 ./benchmark_v1_8 8 v1.7 v1.8`, then `./interleaved_bench.sh ./benchmark_v1_8 ./benchmark_v1_9 8 v1.8 v1.9` — giving two independent 8-round logs on the same machine. Unlike the v1.8 addendum above (measured on a different, noisier 4-thread VM after the original machine became unreachable), these two logs are directly comparable to each other, and the v1.7 numbers below land close to the original dedicated-machine v1.7 section's figures (see comparison at the end of this section).

**A fourth benchmark-methodology pitfall, found this round: execution-order bias.**

Three pitfalls were already fixed and are documented in `benchmark.cpp`'s file header (carried into `benchmark_v1_7.cpp` / `benchmark_v1_8.cpp` for consistency across the interleaved chain):
1. **Allocation elision** — on Windows (MinGW-w64 GCC 16.1.0), the `system` (`new`/`delete`) single-threaded loop was silently optimized away entirely, producing a nonsensical `inf Mops/sec` reading. Root cause: the loop body has no observable side effect (`Player` is never read before being destroyed, and `"bench"`'s 5 characters fit inside `std::string`'s small-string-optimization buffer, so construction never touches the heap either), so the standard permits eliding it — and GCC 16.1 does, where GCC 8.5.0 (Linux) doesn't. Fixed with `volatile long long sink`, accumulating `p->id` after every construction: a standard-mandated observable side effect no compiler may eliminate at any optimization level.
2. **`-Wunused-but-set-variable`** — once `sink` existed, GCC 16.1 flagged it as set-but-never-read (a separate static-analysis diagnostic, unrelated to the elision guarantee `volatile` itself provides). Fixed by printing `sink` in the result line, which doubles as a cheap correctness signal — if a loop didn't run, `sink` stays `0`.
3. **`long` is 32-bit on Windows (LLP64) but 64-bit on Linux (LP64)** — `sink` as `long` silently overflowed (undefined behavior) accumulating tens of millions of iterations on Windows, producing numbers that didn't match Linux. Fixed by using `long long`, guaranteed ≥64-bit on both platforms. A follow-up line-by-line pass over `benchmark.cpp`/`main.cpp` for other platform-dependent-width types found no second instance (`size_t`, `%zu`, `unsigned` are all safe).

The fourth, found while producing the two logs below: `interleaved_bench.sh` alternated *rounds* (round 1: A-then-B, round 2: A-then-B, ...) but always ran the same binary *first* within every individual round. The unmodified `system` control — identical `new`/`delete` code in both binaries of a pair, which should show no systematic difference — was higher in the second-run binary in **8/8 rounds** of the v1.7-vs-v1.8 log below. Since `system`'s code never changed between the two binaries, that can't be a code effect; it's CPU-frequency ramp-up / cache warm-up systematically favoring whichever binary happens to run second within a round. Fixed by flipping which binary runs first on alternating rounds (odd rounds: A then B; even rounds: B then A — see the script's current form), splitting the warm-up advantage evenly instead of concentrating it on one side.

**Results (median of 8 interleaved rounds each, same 6-thread VM, pool ÷ system per round):**

| Scenario | v1.7 | v1.8 | v1.9 |
|---|---|---|---|
| Single-threaded | 2.82× | 1.86× | 1.85× |
| Multi-threaded (6 threads) | 1.78× | 1.24× | 1.66× |

*(The v1.8 column above is from the v1.7-vs-v1.8 log; a second, independent v1.8-vs-v1.9 log measured v1.8 slightly differently — 1.69× single / 1.31× multi — which is run-to-run VM noise between two separate sessions, not a code difference. This is exactly why the comparison that matters is within a single interleaved log, not across two different logs' absolute numbers; see the raw-Mops/sec table below for the v1.8-vs-v1.9 log's own internal comparison.)*

**v1.8 → v1.9 in raw Mops/sec (same log, so directly comparable — this is the number that answers "did switching `in_use` from `vector<bool>` to `vector<uint8_t>` help?"):**

| Scenario | v1.8 median | v1.9 median | Change | Direction consistency |
|---|---|---|---|---|
| Single-threaded (pool) | 95.9 Mops/sec | 112.9 Mops/sec | **+17.7%** | 7/8 rounds favor v1.9 |
| Multi-threaded (pool, 6 threads) | 460.8 Mops/sec | 516.0 Mops/sec | **+12.0%** | 6/8 rounds favor v1.9 |

This is a materially cleaner signal than the previous (4-thread, noisier VM) measurement, where single-threaded showed a smaller +4% with 6/8-round consistency and multi-threaded showed no consistent direction at all (4/8 rounds either way). On this 6-thread VM, both scenarios now show a real, direction-consistent improvement — **`vector<uint8_t>` measurably outperforms `vector<bool>` on both single- and multi-threaded workloads**, not just single-threaded as the earlier, noisier data suggested.

**Comparison against the original dedicated-machine v1.7 numbers:** this run's v1.7 single-threaded ratio (2.82×) lands almost exactly on the original v1.7 section's figure (2.81×, measured on a dedicated 6-hardware-thread machine) — a good sign that this VM is behaving close to dedicated hardware for this workload. The multi-threaded ratio does not match as closely (1.78× here vs. 1.23× originally); a plausible reason is that a shared VM's multi-threaded numbers are more sensitive to host-level scheduling/contention than single-threaded numbers are, but this hasn't been independently confirmed and should be read as a hypothesis, not a finding.

**Open item, unchanged from before this re-run:** optimization B (magic-number fast-division for the `off / blockSize` line, avoiding the CPU division instruction) is still deferred. `blockSize` is confirmed not to be guaranteed a power of two (the current `sizeof(Player)=48 → blockSize=64` case is a coincidence of this particular `T`, not a general property), so a bit-shift substitution isn't valid without a proper magic-number-division implementation. Whether to do B next is an open decision now that A's (`vector<uint8_t>`) benefit is measured cleanly on both scenarios above.

---

## Known Limitations

This project is under active revision. As of the current version (v1.9):
...
* **Deallocation safety checks:** RESOLVED (v1.6 + v1.8) — membership, alignment, and tail-padding checked since v1.6; double-free of a valid block now also checked since v1.8 via the flattened `in_use` array (`vector<uint8_t>` as of v1.9, was `vector<bool>` — see v1.9 section).
* **Hot-path logging / debug split:** RESOLVED (v1.7) — `make debug` toggles `POOL_DEBUG_LOG`; default `make` is silent Release.
* **Benchmark:** RESOLVED (v1.7) — `benchmark.cpp` measures vs the system allocator with real numbers (single ~2.81×, multi ~1.23× on this 6-thread machine). Scope is microbenchmark; see v1.7 honest-scope notes. **`in_use`'s hot-path cost re-benchmarked against v1.7** — see the v1.8 and v1.9 benchmark addenda above; the clean same-machine 6-thread re-run puts v1.8 at ~1.86× single / ~1.24× multi (down from v1.7's ~2.82× / ~1.78×), i.e. `in_use`'s bookkeeping has a real, measured cost, not just a theoretical one (see Next Steps #7, now resolved).
...
7. ~~Re-benchmark v1.8 against v1.7 to quantify the cost (if any) of the `in_use` bookkeeping added to the `allocate()`/`deallocate()` hot path.~~ — DONE via the clean same-machine 6-thread re-run in the v1.9 benchmark addendum above.
8. ~~`main.cpp` still uses `std::vector<bool> in_use` (the v1.8 form)~~ — DONE: `main.cpp`'s `SimpleMemoryPool<T>` now uses `std::vector<uint8_t> in_use`, matching `benchmark.cpp` (see v1.9 section above). Compiles clean with `-Wall -Wextra`; the double-free regression test still needs re-running against this exact file (#9 below).
9. Re-run the double-free regression test (`dfree_test.cpp` — rename to `test_v1_9.cpp`?) against the now-updated `main.cpp` to confirm the `vector<uint8_t>` switch didn't alter double-free-detection behavior.
10. Decide on optimization B (magic-number fast-division for `off / blockSize`) now that A's (`vector<uint8_t>`) benefit is cleanly measured — see the open item at the end of the v1.9 benchmark addendum above.
11. Decide whether to move `benchmark_v1_7.cpp` / `benchmark_v1_8.cpp` out of the default `make` build and into `archive/`, keeping only the current `benchmark.cpp` in the Makefile (proposed, not yet actioned).